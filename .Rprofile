...
# avoid "ERROR: moving to final location failed" on R >= 3.6
# R.Version()$minor >= 6 && Sys.setenv(R_INSTALL_STAGED=FALSE)
# 追記：Microsoft R Open 3.5.3を利用することにしたのでしばらくはコメントアウト
...
setHook(packageEvent("grDevices", "onLoad"), function(...) {
  ...
  if (.Platform$OS.type == "windows") {
    ...
    grDevices::windowsFonts(
      mincho = grDevices::windowsFont("Yu Mincho"),
      gothic = grDevices::windowsFont("Yu Gothic")
    );
